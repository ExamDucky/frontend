...
class ExamManagementRightSection extends StatefulWidget {
  const ExamManagementRightSection({super.key, required this.studentList});

  final List<StudentExamAttemptModel> studentList;
    ...
    return Expanded(
      child: DataTable2(
          columns: const [
            ...
            DataColumn2(
                label: Center(child: Text("exam submitted")),
                size: ColumnSize.L),
            DataColumn2(
                label: Center(child: Text("plagiarism")), size: ColumnSize.M),
            DataColumn2(
                label: Center(child: Text("valid")), size: ColumnSize.S),
          ...
          rows: studentList
              .map((s) => DataRow(cells: [
                    ...
                    DataCell(Center(
                      child: s.examSubmitted,
                    )),
                    DataCell(Center(
                      child: s.plagiarismPercent,
                    )),
                    DataCell(Center(
                      child: s.isValid,
                    )),
